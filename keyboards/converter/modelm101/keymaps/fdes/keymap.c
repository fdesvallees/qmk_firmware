...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [0] = LAYOUT( /* Base layer */
    KC_ESC,           KC_BRMD, KC_BRMU,   KC_F3,   KC_F4,   KC_F5,   KC_F6,   KC_F7,   KC_F8,   KC_F9,   KC_KB_MUTE,  KC_KB_VOLUME_DOWN,  KC_KB_VOLUME_UP, LSFT(LGUI(KC_4)), KC_SLCK, KC_PAUSE,
    ...
    KC_LCTL,           KC_LALT,                           KC_SPC,                                 KC_RALT,               MO(1),   KC_LEFT, KC_DOWN, KC_RGHT,   KC_P0,           KC_PDOT
  ),
  [1] = LAYOUT(
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  {
    case RESET:
      if (record->event.pressed)  // Switch-on all 3 LEDs when RESET is pressed
      {
        writePinLow(B4);
        writePinLow(B5);
        writePinLow(B6);
